...
void make_int_arr(t_file *file, int r, int c)
{
	...
	while (r >= 0)
	{
		...
		while (c < (int)ft_strlen(file->map.map_arr[r]))
		{
			...
			// if (file->map.map_int_arr[i][c] == 0 || file->map.map_int_arr[i][c] == 1)
			// 	printf("%d", file->map.map_int_arr[i][c]);
			// else if (file->map.map_int_arr[i][c] == 32)
			// 	printf(" ");
			c++;
		}
		// printf("\n");
			// printf("row %d length : %d\n", r, (int)ft_strlen(file->map.map_arr[r]));
		i++;
		r--;
	}
	// r = file->map.row - 1;
	// for (int i = 0; i < file->map.row; i++)
	// {
	// 	for (int c = 0; c < (int)ft_strlen(file->map.map_arr[r]); c++)
	// 	{
	// 		if (file->map.map_int_arr[i][c] == 0 || file->map.map_int_arr[i][c] == 1)
	// 			printf("%d", file->map.map_int_arr[i][c]);
	// 		else if (file->map.map_int_arr[i][c] == 32)
